...
{
    class MarkUpStrategy : ISudokuStrategy
    {
        ...
        public int[,] Solve(int[,] sudokuBoard)
        {
            for (int row = 0; row < sudokuBoard.GetLength(0); row++)
            {
                for (int col = 0; col < sudokuBoard.GetLength(1); col++)
                {
                    ...
                    {
                        var possibilitiesInRowAndCol = GetPossiblitiesInRowAndCol(sudokuBoard, row, col);
                        var possibilitiesInBlock = GetPossibilitiesInBlock(sudokuBoard, row, col);
                        sudokuBoard[row, col] = GetPossibilityIntersections(possibilitiesInRowAndCol, possibilitiesInBlock);
                    }
                }
            }
        ...
        private int GetPossiblitiesInRowAndCol(int[,] sudokuBoard, int givenRow, int givenCol)
        {
            ...
            return Convert.ToInt32(String.Join(string.Empty, possibilities.Select(p => p).Where(p => p != 0))); //retourne tous les chiffres possibles "1234" 
        }
        ...
        private int GetPossibilityIntersections(int possibilitiesInRowAndCol, int possibilitiesInBlock)
        {
